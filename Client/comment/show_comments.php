...
while($comment = mysqli_fetch_assoc($commentsResult)){
	$commentHTML .= '
		...
		<div class="panel-footer" align="right"><button type="button" class="btn btn-primary reply" style="width:10%" id="'.$comment["id"].'">Reply</button></div>
		</div> ';
...
function getCommentReply($conn, $parentId = 0, $marginLeft = 0) {
	...
	if($commentsCount > 0) {
		while($comment = mysqli_fetch_assoc($commentsResult)){  
			$commentHTML .= '
				<div class="panel panel-primary" style="margin-left:'.$marginLeft.'px; width:75%">
				<div class="panel-heading">By <b>'.$comment["sender"].'</b> on <i>'.$comment["date"].'</i></div>
				<div class="panel-body">'.$comment["comment"].'</div>
				<div class="panel-footer" align="right" ><button type="button" class="btn btn-primary reply" style="width:10%  background-color: #4CAF50;" id="'.$comment["id"].'">Reply</button></div>
				</div>
				';
